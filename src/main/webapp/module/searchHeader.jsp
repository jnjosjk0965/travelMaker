<%@page import="com.java.mapping.TravelClassMapper"%>
...
String originCode = CityMapper.getAirportCode(info.getOriginLocation());
String destinationCode = CityMapper.getAirportCode(info.getDestinationLocation());
%>
<style>
	.search-header-root{
		min-height: 3.5rem;
...
<div class="search-header-root">
	<section class="r-flex search-container">
		<div class="mx-3 pt-1">
			<a href="searchResult.jsp" class="btn btn-primary">
		...
		<div class="r-flex" style="order:2; flex-grow: 1;">
			<div class="pt-1">
				<div class="r-flex">
					<span><%=CityMapper.getName(originCode) %></span>
					<span>(<%=originCode%>)</span>
					<span>-</span>
					<span><%=CityMapper.getName(destinationCode)%></span>
					<span>(<%=destinationCode%>)</span>
				</div>
				<div class="r-flex mt-1">
					<span class="me-1 pe-1" style="box-shadow: inset -1px 0 0 0 #e0e3e5;"><%=info.getNoP()%> 승객</span>
					<span><%=TravelClassMapper.getTravelClassKor(info.getTravelClass())%></span>
				</div>
			</div>
